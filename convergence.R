...
mcpidf <- mcpidf %>% pivot_wider(id_cols=countryid, names_from = Time, values_from = accmcpi, names_prefix = "year")
...
club1 <- pivot_wider(club1, id_cols=year,names_from = countryid, values_from = mcpi,names_prefix = "country")
club1 <- club1 %>% na.exclude()

beta_conv(club1,time_0=1975, time_t=2018, timeName = "year")                     

club2 <- mcpidf %>% filter(club==2)
club2 <- pivot_wider(club2, id_cols=year,names_from = countryid, values_from = mcpi,names_prefix = "country")
club2 <- club2 %>% na.exclude()

beta_conv(club2,time_0=1975, time_t=2018, timeName = "year") 

club3 <- mcpidf %>% filter(club==3)
club3 <- pivot_wider(club3, id_cols=year,names_from = countryid, values_from = mcpi,names_prefix = "country")
club3 <- club3 %>% na.exclude()

beta_conv(club3,time_0=1975, time_t=2018, timeName = "year") 

club4 <- mcpidf %>% filter(club==4)
club4 <- pivot_wider(club4, id_cols=year,names_from = countryid, values_from = mcpi,names_prefix = "country")
club4 <- club4 %>% na.exclude()

beta_conv(club4,time_0=1975, time_t=2018, timeName = "year") 

club5 <- mcpidf %>% filter(club==5)
club5 <- pivot_wider(club5, id_cols=year,names_from = countryid, values_from = mcpi,names_prefix = "country")
club5 <- club5 %>% na.exclude()

beta_conv(club5,time_0=1975, time_t=2018, timeName = "year") 

club6 <- mcpidf %>% filter(club==6)
club6 <- pivot_wider(club6, id_cols=year,names_from = countryid, values_from = mcpi,names_prefix = "country")
club6 <- club6 %>% na.exclude()

beta_conv(club6,time_0=1975, time_t=2018, timeName = "year") 
...
mcpidf <- datamcpi %>% select(countryid, Time, mcpi, accmcpi)
colnames(mcpidf) <- c("countryid", "year", "mcpi", "accmcpi")
...
club_mcpi <- findClubs(data.frame(mcpidf),dataCols = 2:45, unit_names = 1, refCol = 45, time_trim = 1/3,
                        cstar_method = "incremental",cstar=0.8,cstar_increment = 0.1, cstar_cap = 5)
summary(clubs_mcpi)

mcclubs <- mergeClubs(clubs_mcpi_fix, mergeMethod = "PS")
summary(mcclubs)

mcclubs2 <- mergeClubs(mcclubs, mergeMethod = "PS")
summary(mcclubs2)
